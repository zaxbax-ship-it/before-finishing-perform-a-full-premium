...
Search engines, canonicals, Open Graph URLs, `robots.txt`, and `sitemap.xml` all rely on `NEXT_PUBLIC_SITE_URL`, so that variable must match the final production domain.
