...
export function LifeOfferModal({ t, cost, accept, decline }: { t: Record<string, string>; cost: number; accept: () => void; decline: () => void }) {
  ...
  useDialogFocus(true, dialogRef, decline);
  return (
    <div className="modal-backdrop" ref={dialogRef} role="dialog" aria-modal="true" aria-labelledby="life-offer-title">
      <div className="glass modal-card">
        ...
        <div className="mt-5 flex gap-3">
          ...
          <button className="ghost-button focus-ring inline-flex flex-1 items-center justify-center gap-2" type="button" data-autofocus onClick={decline}>
            <CloseIcon size={16} />
            {t.lifeOfferDecline}
          </button>
        </div>
